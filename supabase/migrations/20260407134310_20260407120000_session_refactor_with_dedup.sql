/*
  # Session-based refactor with duplicate deduplication
  ...
  4. Deduplication: Keep only the latest completion per task/date

  5. Idempotent default-task seeding
...
-- 0. Dedup: Delete all but the latest per task/date
DELETE FROM completions
WHERE id NOT IN (
  SELECT DISTINCT ON (task_id, date) id
  FROM completions
  WHERE status = 'completed'
  ORDER BY task_id, date, completed_at DESC
);
